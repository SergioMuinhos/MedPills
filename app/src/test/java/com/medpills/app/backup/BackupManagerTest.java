...
public class BackupManagerTest {
    ...
    public void testParseAndImportBackupJson() throws Exception {
        ...
        db.runInTransaction(() -> {
            ...
            // Insert profiles
            for (com.medpills.app.database.Profile profile : payload.profiles) {
                db.profileDao().insert(profile);
            }

            // Insert medications
            for (com.medpills.app.database.Medication med : payload.medications) {
                db.medicationDao().insert(med);
            }

            // Insert schedules
            for (com.medpills.app.database.Schedule sched : payload.schedules) {
                db.scheduleDao().insert(sched);
            }

            // Insert intake logs
            for (com.medpills.app.database.IntakeLog log : payload.intakeLogs) {
